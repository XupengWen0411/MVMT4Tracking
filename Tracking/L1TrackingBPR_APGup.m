function [track_res,output,img_map] = L1TrackingBPR_APGup(s_frames, paraT)
...
[T,T_norm,T_mean,T_std,img_map] = InitTemplates(sz_T,nT,img,init_pos);%初始化模板尺寸、模板数量、图像、模板截取初始坐标
...
for t = 1:nframes
    ...
    while (n<n_sample)&&(q(n)>=tau)        
        ...
        elseif(p(indq(n))>eta_max)
            id_max	= indq(n);%可能性最大的测试样本的ID
            c_max	= c;%可能性最大的稀疏表示系数
            eta_max = p(indq(n));
